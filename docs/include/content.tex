...
docker build --build-arg GH_TOKEN=<REPLACE-ME> --build-arg username=<REPLACE-ME> --build-arg email=<REPLACE-ME> -t quarkus_image .
            \end{lstlisting}
            \item Before you are able to create and run the container, you have to create a seperate network for your container, for example to connect a database (\ref{sec:database}). \label{item:network}
            \begin{lstlisting}[language=bash]
docker network create --driver bridge quarkus-net
            \end{lstlisting}
...
gpg --batch --gen-key gen-key-script
    ...
    \section{How to set up a database container}\label{sec:database}
        ...
        \begin{enumerate}
            \item Create and run a PostgreSQL database. The quarkus-net network has to be already created (look at \ref{item:network}):
            \begin{lstlisting}[language=bash, breaklines=true]
